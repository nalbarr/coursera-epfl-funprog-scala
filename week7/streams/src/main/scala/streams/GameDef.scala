...
trait GameDef {
  ...
  // #3
  // NAA.
  // 1. Simple initialization of block (standing up)
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    // #4
    // NAA.
    // 1. populate List as neighbors with representations as pairs of Block, Move
    // types for all possible cases of left, right, up, and down.  This pair
    // represents both block behavior and move type encoding (e.g., an enumeration)
    ...
    // #5
    // NAA.
    // 1. Filter list based on invoking isLegal behavior using
    // pair wise/tuple place sholder notation  
    ...
    // #6
    // NAA
    // 1. Since b1 < b2 lexigraphically, the only valid case where it is not is when
    // it standing and not taking up 2 cube spaces
    ...
    // #7
    // NAA
    // 1. Since a block is represented by 2 cubes b1 and b2, a legal move
    // can decomposed in make sure that both cubes b1 and b2 are on the
    // terrain and be represented by individual calls to terrain() function
    // (and always assumes b1 < b2 lexigraphically) as assumed in a
    // stream based text encoding where b1 need to be always before b2 in
